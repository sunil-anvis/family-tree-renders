...
function init3DTree() {
    ...
    const getParams = () => {
        switch (currentIsoAngle) {
            case 'top': return { angle: 0, tilt: 1.0, zScale: 0, billboard: false, labels: 'flat' }; // Max Flat (with slight Z for layering)
            case 'front': return { angle: 0, tilt: 1.0, zScale: 1, billboard: false, labels: 'flat' }; // Max Flat (with slight Z for layering)
            case 'iso':
            default: return { angle: Math.PI / 4, tilt: 0.5, zScale: 1.0, billboard: false, labels: 'iso' }; // Standard Iso
        }
    };
    ...
    const genColors = [
        "#7C6EE6", // Soft Royal Purple (Me)
        "#5EC2B7", // Calm Teal
        "#F4A261", // Warm Soft Orange
        "#6CA0DC", // Muted Blue
        "#E5989B", // Soft Rose
        "#A3B18A"  // Sage Green
...
Z`;
    ...
    const linkPathGenerator = (d) => {
        ...
        // Center of Source Block
        // We need to account for spouse offset if present to center the line
        const AVATAR_Gap = 110;

        // If s has spouse, center is mid of s.x and s.x + offset?
        // Node group is at s.x. Spouse at s.x + 110 (effectively).
        // Center of pair = s.x + 55.
        // If no spouse, center = s.x.

        let linkSx = sx;
        // if (s.data.spouse) linkSx += AVATAR_Gap / 2; // Keep simple for now

        const tx = t.x, ty = t.y;
        let linkTx = tx;
        // if (t.data.spouse) linkTx += AVATAR_Gap / 2;
    ...
    nodes.each(function (d) {
        ...
        const renderAvatar = (data, isSpouse) => {
            ...
            const localIso = (dx, dy) => [(dx - dy) * avatarScale, (dx + dy) * 0.5 * avatarScale];
            ...
            const activeMatrix = (params.labels === 'billboard') ? billboardMatrixStr : matrixStr;
